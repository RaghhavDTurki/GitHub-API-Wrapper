...
import { ListRepoTopicsInput } from "../../types/InputTypes";
import { Octokit } from "octokit";

export const listRepoTopics = async (req: Request, res: Response) => {
    try {
        const body = req.body as ListRepoTopicsInput;
        if (body.username) {
        }
        ...
        if (body.username) {
            const repoTopics = await githubClient.rest.repos.getAllTopics({
                owner: body.username,
                repo: body.repo_name
            });
            if (!repoTopics) {
